...
Description:  This plugin will add heart like button to posts and number of likes, saves number of likes of each user for each post so he can't like it more than once
Version:      1.0
Author:       Andrei Miheev
License:      GPL2
*/

add_action( 'wp_enqueue_scripts', 'lbp_include_assets' );
function lbp_include_assets() {
	wp_enqueue_style('lbp-style', plugins_url('lbp-style.css', __FILE__), false, null, 'all' );
	wp_enqueue_script( 'lbp-script', plugins_url('lbp-script.js', __FILE__), array('jquery'), null, true );
	wp_localize_script( 'lbp-script', 'lbp', array( 'ajaxurl' => admin_url('admin-ajax.php') ) );
}

add_shortcode( 'like-button', 'lbp_add_like_button');
function lbp_add_like_button() {
	ob_start();
	global $post;
  $user_already_liked_this_post = false;
	$lbp_likes = get_post_meta($post->ID, 'lbp_likes', true); // empty string returned if no such key
  $lbp_likes = ($lbp_likes == "") ? 0 : $lbp_likes;
  $user_id = get_current_user_id();
  $liked_posts = get_user_meta($user_id, 'liked_posts');
  foreach ($liked_posts as $liked_post) {
    if ($liked_post == $post->ID) {
      $user_already_liked_this_post = true;
    }
  }
	?>
	<div class="block">
		...
		<svg
      id="lbp-like-btn"
			class="heart-icon <?php if ($user_already_liked_this_post) echo 'isLiked' ?>"
      data-post_id="<?php echo $post->ID; ?>"
      data-nonce="<?php echo wp_create_nonce('lbp-nonce-name'); ?>"
      data-user_id="<?php echo $user_id; ?>"
		...
		<!-- counter -->
		<span class="number-of-likes"><?php echo $lbp_likes; ?></span>
	</div>
...
add_action( 'wp_ajax_lbpgo', 'lbp_add_like' );
add_action( 'wp_ajax_nopriv_lbpgo', 'lbp_add_like' );
function lbp_add_like() {
	check_ajax_referer( 'lbp-nonce-name', 'security' ); // will die automatically if not

  $liked = (bool)$_POST['isLiked']; // read 0 or 1 and transform it to boolean ( php does it even without (bool) )
  $post_id = $_POST['post_id'];
  $user_id = $_POST['user_id'];

	$like_count = get_post_meta($post_id, "lbp_likes", true);
  $like_count = ($like_count == '') ? 0 : $like_count;
  if ($liked) {
    $new_like_count = $like_count + 1;
    add_user_meta($user_id, 'liked_posts', $post_id); // needed add instead of update, bcs update overwrites value, add adds another with the same key
  } else {
    $new_like_count = $like_count - 1;
    // delete number of liked post from user metadata
    delete_user_meta($user_id, 'liked_posts', $post_id);
  }

  // update post's number of likes
  $res = update_post_meta($post_id, 'lbp_likes', $new_like_count); // if no such key exists, will be added and its id returned
